...
As a result, the project builds and its unit tests run on any machine, including CI, with no client files. Unit tests use small definition fixtures written by the project. Tests that need a real install carry the CTest label `client` and run only when `AMBROSE_CLIENT_DIR` is set; tests that need the user's own type dump also carry the label `client` and run only when `AMBROSE_TYPE_DUMP_PATH` names it. That dump is either the older reference dump or one typeextract wrote, and the two differ: the extraction finds five classes and four properties the reference missed, and writes sixty enum option values as the empty text the client holds where the reference wrote 0. A test that pins class or property counts therefore reads the revision and extractor from the dump's own header and expects the shape whoever wrote it produces, so the same check passes on either dump and a dump of another revision is skipped rather than failed against counts that were never its own. The compact codec's capture check also needs `AMBROSE_OBJECT_SAMPLES_DIR`, a folder of blobs captured on the user's own machine and named after their class, which are never committed. `AMBROSE_FUZZ_ITERATIONS` sets how many mutations the decoder fuzz test runs. typeextract's client tests also extract a second install when `AMBROSE_SECOND_CLIENT_DIR` names one, so type discovery is checked on another client revision.
