 ...
 * Tests the operators who sign in to the panel: a name is held to the account rules and cannot be taken twice or differ from another only by case, a password is held to one policy on every path that sets one and is never kept except as an Argon2id hash, an unknown user, a disabled user and a wrong password answer the same and take about the same time, a password change or a disable bumps the generation that ends that user's other sessions, and the store says when it holds no user at all so the supervisor knows to make an owner.
 */
