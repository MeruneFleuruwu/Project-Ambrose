 ...
 * A small HTTP/1.1 client for one app's admin API on this machine: it sends one request with the app's bearer token, a Host header the listener accepts and the caller's request id, reads the whole answer before a deadline, and reports the status, content type and body, or why no answer came, so the supervisor reads health, asks for a shutdown and relays the panel's requests without a library of its own.
 */
...
public:
    ...
    AdminClient(std::string host, uint16 port, std::string token);
