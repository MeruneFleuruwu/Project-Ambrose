...
# Checks status fixtures and prevents required status fields from being removed or renamed.
...
def schema_properties(definition: dict[str, Any], path: str = "$") -> set[tuple[str, str]]:
    names = {(path, name) for name in definition.get("properties", {})}
    for name, child in definition.get("properties", {}).items():
        names.update(schema_properties(child, f"{path}.{name}"))
    if "items" in definition:
        names.update(schema_properties(definition["items"], f"{path}[]"))
    return names
...
def main() -> None:
    ...
    removed_properties = schema_properties(v1) - schema_properties(v2)
    if removed_required:
    ...
    print("validated 2 status fixtures; v2 preserves all v1 required fields and properties")
